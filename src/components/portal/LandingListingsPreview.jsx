...
const REGION_LABELS = {
  BA: 'Bratislava', TT: 'Trnava', TN: 'Trenčín', NR: 'Nitra',
  ZA: 'Žilina', BB: 'Banská Bystrica', PO: 'Prešov', KE: 'Košice',
};
...
const fmt = (n) =>
  n > 0 ? new Intl.NumberFormat('sk', { maximumFractionDigits: 0 }).format(n) + ' €' : 'Na dopyt';
    ...
    >
      <div className="relative h-44 bg-slate-100 overflow-hidden">
        {img ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-slate-200 text-5xl bg-gradient-to-br from-slate-50 to-slate-100">
            🏠
          </div>
        )}
        ...
        <div className="absolute top-2.5 left-2.5 flex gap-1.5">
          ...
          {listing.visibility === 'off_market' && (
            <span className="text-xs font-bold bg-violet-600 text-white px-2.5 py-1 rounded-full shadow-sm">
              Off-market
            </span>
          )}
        </div>
      ...
      <div className="p-4">
        <div className="font-bold text-slate-800 text-sm leading-snug line-clamp-1 mb-2 group-hover:text-indigo-600 transition-colors">
          {listing.title}
        </div>
        <div className="flex items-center gap-1 text-slate-400 text-xs mb-3">
          ...
          <span>
            ...
            {listing.location_region ? `, ${REGION_LABELS[listing.location_region]}` : ''}
          </span>
        </div>
...
export default function LandingListingsPreview({ onLogin }) {
  ...
  const { data: listings = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      base44.entities.Listing.filter({ status: 'active', visibility: 'public' }, '-created_date', 20),
  });
